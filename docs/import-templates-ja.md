...
**重要なお知らせ**：システムは現在 **v3 統一テンプレート形式** をサポートしており、より強力な式エンジン、統一されたフィールド命名体系、柔軟なメディア検出メカニズムを提供します。詳細な v3 テンプレートの説明については、[v3 テンプレートシステムドキュメント](v3-templates-ja.md)を参照してください。

### コア機能：
- 複数のフロントエンド形式のデータファイルのインポートをサポート（v2 および v3 テンプレート）
- 異なる形式のデータファイル（XML、テキスト）の自動認識と処理
- インテリジェントなメディアファイルのマッチングと関連付け（mediaDiscovery 自動検出をサポート）
- 柔軟なフィールドマッピング設定（nomcourt / dbColumn / javaField の3つの命名をサポート）
- ヘッダー情報の抽出と処理のサポート
- 複数値フィールドの処理
- ファイル拡張子の自動認識
- 式エンジンのサポート（関数呼び出し、演算子、条件式）
- 計算変数の自動注入（filename、filepath）
...
- `transform`：オプション、フィールド値変換規則、以下のオプションをサポート：
  ...
  - `replace`: 文字列置換（`from` と `to` フィールドを持つオブジェクト）、正規表現をサポート。`from` は正規表現パターン、`to` は置換文字列（`$1` などのグループ参照をサポート）
...
```
この設定は、絶対パス `/home/pi/ROMs/nes/mm2.nes` を相対ファイル名 `mm2.nes` に変換します
...
インポート機能は、外部フロントエンドからゲームデータをインポートするための柔軟で拡張可能な方法を提供し、複数のフロントエンド形式をサポートして様々なユーザーのニーズに応えます。インポートテンプレートを通じて、ユーザーは異なるフロントエンドのデータを迅速に移行するためのインポートルールを簡単に構成できます。

**v3 テンプレート形式の使用を推奨します**。より強力な式エンジン、統一されたフィールド命名、自動メディア検出機能をお楽しみいただけます。
...
#### v1.3 (2026-09-13)
- v3 統一テンプレート形式サポートを追加
- 式エンジンのサポート（関数呼び出し、演算子、条件式）
- 統一されたフィールド命名体系（nomcourt / dbColumn / javaField）
- 新しい mediaDiscovery 自動メディア検出メカニズム
- 計算変数の自動注入（filename、filepath）
- 50 種の ScreenScraper 公式メディアタイプをサポート
