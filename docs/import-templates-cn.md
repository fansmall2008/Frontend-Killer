...
**重要提示**：系统现已支持 **v3 统一模板格式**，提供更强大的表达式引擎、统一的字段命名体系和灵活的媒体发现机制。详细的 v3 模板说明请参考 [v3 模板系统文档](v3-templates-cn.md)。

### 核心功能：
- 支持多种前端格式的数据文件导入（v2 和 v3 模板）
- 自动识别和处理不同格式的数据文件（XML、文本）
- 智能媒体文件匹配和关联（支持 mediaDiscovery 自动发现）
- 灵活的字段映射配置（支持 nomcourt / dbColumn / javaField 三种命名）
- 支持表头信息提取和处理
- 多值字段处理
- 自动识别文件扩展名
- 表达式引擎支持（函数调用、运算符、条件判断）
- 计算变量自动注入（filename、filepath）
...
- `transform`：可选，字段值转换规则，支持以下选项：
  ...
  - `replace`: 字符串替换（对象包含 `from` 和 `to` 字段），支持正则表达式。`from` 为正则表达式模式，`to` 为替换字符串（支持 `$1` 等分组引用）
...
```
此配置可将绝对路径 `/home/pi/ROMs/nes/mm2.nes` 转换为相对文件名 `mm2.nes`
...
**推荐使用 v3 模板格式**，享受更强大的表达式引擎、统一的字段命名和自动媒体发现功能。
...
#### v1.3 (2026-09-13)
- 新增 v3 统一模板格式支持
- 支持表达式引擎（函数调用、运算符、条件判断）
- 支持统一的字段命名体系（nomcourt / dbColumn / javaField）
- 新增 mediaDiscovery 自动媒体发现机制
- 支持计算变量自动注入（filename、filepath）
- 支持 50 种 ScreenScraper 官方媒体类型
